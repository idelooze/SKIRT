...
/** PowerLawExpTailGrainSizeDistribution is a GrainSizeDistribution subclass that represents a
    dust grain size distribution of the form \f[ \Omega(a) = (\frac{\text{d}n_\text{D}}{\text{d}a})
    / n_\text{H} = C \,a^{\alpha} \,f_\text{ed}(a) \qquad\text{for}\quad
    ...
    \right) & \quad a>a_\text{t} \end{cases} \f]. \f]
...
{
    Q_OBJECT
    Q_CLASSINFO("Title", "a power-law with exp tail dust grain size distribution")
